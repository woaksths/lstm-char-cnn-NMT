...
class NMT(nn.Module):
    ...
    def __init__(self, word_embed_size, hidden_size, vocab, dropout_rate=0.3, no_char_decoder=False):
        ...
        self.model_embeddings_source = ModelEmbeddings(self.word_embed_size, vocab.src)
        self.model_embeddings_target = ModelEmbeddings(self.word_embed_size, vocab.tgt)

        self.hidden_size = hidden_size
        self.dropout_rate = dropout_rate
        self.vocab = vocab
        self.vocab_entry = VocabEntry()
        ### COPY OVER YOUR CODE FROM ASSIGNMENT 4
        
        self.encoder = nn.LSTM(self.word_embed_size, self.hidden_size, bidirectional=True)
        self.decoder = nn.LSTMCell(self.word_embed_size+self.hidden_size, self.hidden_size)        
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, self.device) # max_sentence_lengths, batch, max_word_lengths
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        src_embedded = self.model_embeddings_source(source_padded)      
        src_embedded = pack_padded_sequence(src_embedded, source_lengths)
        enc_hiddens, (h_n, c_n) = self.encoder(src_embedded)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings_target(target_padded)  # max_sentence_length, batch, embedding_dim 
