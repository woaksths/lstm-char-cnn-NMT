...
class CharDecoder(nn.Module):
    ...
    def forward(self, input, dec_hidden=None):
        ...
        outputs, dec_hidden = self.charDecoder(embedded, dec_hidden)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        padding_token_idx = self.target_vocab.char_pad
        inputs = char_sequence[:-1, :]
        targets= char_sequence[1:, :] # length, batch 
        scores, dec_hidden = self.forward(inputs, dec_hidden) 
        loss = nn.CrossEntropyLoss(reduction ='sum', ignore_index=padding_token_idx)
        losses = loss(scores.permute(1,2,0), targets.transpose(0,1))
        return losses
        ### END YOUR CODE
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        sos_id = self.target_vocab.start_of_word
        eos_id = self.target_vocab.end_of_word
        batch_size = initialStates[0].size(1)
        current_char = torch.tensor([sos_id] * batch_size, device=device).view(1,batch_size)
        dec_hidden = initialStates
        output_word = ['{' for _ in range(batch_size)]
        
        for i in range(max_length):
            scores, dec_hidden = self.forward(current_char, dec_hidden)
            scores = torch.softmax(scores, dim=-1)
            current_char = torch.argmax(scores, dim=-1)
            for idx, vocab_idx in enumerate(current_char.squeeze(0)):
                if output_word[idx][-1] == '}':
                    continue
                output_word[idx] += self.target_vocab.id2char[vocab_idx.item()]
        return [output.replace('{','').replace('}','') for output in output_word]
        ### END YOUR CODE
